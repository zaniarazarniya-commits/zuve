...
function DetailsFormScreen({
  ...
  const isValid = phone.length >= 7 && eta !== "";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/guest/${token}`, {
        ...
        body: JSON.stringify({ guest_phone: phone, eta, notes }),
      });
...
function BookingOverview({
  ...
  return (
    <div className="min-h-full px-5 pt-12 pb-8 content-in">
      <div className="max-w-lg mx-auto space-y-8">
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm space-y-5">
          ...
          {booking.notes && (
            <>
              <div className="h-px bg-sand-light/60" />
              <div>
                <p className="text-[10px] font-medium uppercase tracking-wider text-granite mb-1">Anteckningar</p>
        ...
        {/* CTA till upplevelser */}
        <button
          onClick={onContinue}
          className="w-full py-4 rounded-2xl bg-primary text-white text-sm font-medium transition-all duration-500 active:scale-[0.98]"
        >
          Utforska din vistelse
        </button>
      </div>
    </div>
  );
...
function ExploreScreen({ token }: { token: string }) {
...
export default function GuestPage() {
  ...
  return (
    <main className="min-h-full bg-background relative">
      {step >= 3 && <ExploreScreen token={token!} />}
      {step >= 2 && step < 3 && (
        <BookingOverview booking={booking} token={token!} onContinue={() => setStep(3)} />
      )}
      {step >= 1 && step < 2 && (
        <DetailsFormScreen
          token={token!}
