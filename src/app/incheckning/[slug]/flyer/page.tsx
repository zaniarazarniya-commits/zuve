...
//
// QR-koden byggs från webbläsarens egen adress, så skylten pekar
// alltid på samma domän som du själv öppnade sidan på.
// ============================================================
...
export default function CheckinFlyerPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        if (!res.ok) {
          setError(data.error ?? "Kunde inte hämta gruppen.");
          return;
        }
        setGroup(data.group);
        // window finns bara i webbläsaren, därför sätts adressen här
        // och inte under renderingen.
        setCheckinUrl(`${window.location.origin}/incheckning/${slug}`);
      } catch {
        if (!cancelled) setError("Kunde inte hämta gruppen.");
      }
    }
    ...
    <>
      ...
      <div className="flyer-wrap">
        ...
        <div className="flyer-page">
          ...
          >
            {group ? formatDate(group.checkIn) : " "}
          </p>
